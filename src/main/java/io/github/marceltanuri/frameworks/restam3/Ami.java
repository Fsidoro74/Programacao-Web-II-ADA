...
public final class Ami {
    ...
    public static void start(int port) {
        ...
        try {
            ...
            // Ensure process stays alive until it is terminated; register graceful shutdown
            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                try {
                    System.out.println("[Ami] Shutdown signal received. Stopping server...");
                    if (server != null) {
                        server.stop(0);
                    }
                } finally {
    ...
    /**
     * Blocks the current thread until the JVM is shutting down.
     * This avoids using Thread.sleep to keep the process alive.
     */
